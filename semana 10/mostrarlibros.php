...
// del registro
    if (isset($_POST['guardar'])) {
        ...
        $isbnx = isset($_GET['id']) ? addslashes(trim($_GET['id'])) : "";
...
// en la tabla libros
    ...
    while ($row = $resultc->fetch_assoc()) {
        ...
        echo "\t\t\t<td>{$row['isbn']}</td>\n";
        echo "\t\t\t<td>" . stripslashes($row['autor']) . "</td>\n";
        echo "\t\t\t<td>" . stripslashes($row['titulo']) . "</td>\n";
        echo "\t\t\t<td>\${$row['precio']}</td>\n";
        echo "\t\t\t<td align=\"center\">\n";
        echo "\t\t\t\t<a href=\"{$_GET['opc']}.php?id={$row['isbn']}\">Editar</a>\n";
        echo "\t\t\t\t|\n";
        echo "\t\t\t\t<a href=\"?del=s&id={$row['isbn']}\">Eliminar</a>\n";
        echo "\t\t\t</td>\n";
        echo "\t\t</tr>\n";
    }
    echo "\t</tbody>\n";
    echo "\t<tfoot>\n";
    echo "\t\t<tr id=\"tfooter\">\n";
    echo "\t\t\t<th colspan=\"3\">Número de registros: $num_results.</th>\n";
    echo "\t</tr>\n";
    echo "\t</tfoot>\n";
    echo "</table>\n";
    ?>
